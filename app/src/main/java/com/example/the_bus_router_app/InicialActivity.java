...
public class InicialActivity extends ListActivity {
    ...
    private List<? extends Map<String,?>> dadosToMap(ArrayList<Rota> listaRotas) {
        List<HashMap<String,String>> lista = new ArrayList<>();

        for (int i = 0; i < listaRotas.size(); i++) {
            HashMap<String, String> item = new HashMap<>();
            item.put("id", String.valueOf(listaRotas.get(i).id));
            item.put("transportadora", listaRotas.get(i).transportadora);
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        telaDetalhesIntent.putExtra("transportadora", rota.transportadora);
